...
**Revision 226** — supersedes Revision 225 and earlier. **Seven findings reach `resolved`, and the first bundle of this session closes.** `0036` F1 (option (iv), scanning `docs/`), `0039` F7, F12, F15 and F16 (typed edges, the transition rules, the citation check, the completeness check and the one-way guard) and `0043` F5 and F6 (authority moved to data, repeatable fields as arrays). Three `resolutions.md` files record what was actually done, with the revision and commit for each. `0036` derives to `resolved` — one finding, resolved — and `0039` F8 moves `framing → decided`, which D6 should have done on 2026-09-07 and did not. **Why none of this was recorded when it happened is F17: resolving has no procedure.** Superseding has nine numbered steps, reopening and withdrawing have theirs, transferring has one; the commonest transition in the framework, and the only lifecycle event that produces a tracked file, had a file schema in §11 and no instructions anywhere. Eighteen decisions were accepted and seven carried out across Revisions 221–225, and not one row was written or status moved until the owner asked what had been resolved — the answer at that moment being *nothing*, over a tree that had absorbed five revisions of the work. D18 installs §9b: carry out the decision, write the row, move the status, in that order, because the row is the evidence the status asserts. F18 records the word this session invented under pressure because the framework has none — `accepted` cannot say whether a decision's work was carried out, so D5 with 31 edges in the data and D6 with nothing written render identically. Left `framing`: an eighth Outcome would put a progress fact into a vocabulary D9 defined as acts performed on a decision, and §9b may make *carried out* derivable instead. The check for a decided finding with no row is deliberately not written this revision — three checks landed today and every one failed against a healthy tree on its first run.

**Revision 225** — supersedes Revision 224 and earlier. The extraction left markdown in the data: **162 contaminated fields across 47 of the 54 files**, every `recordedOn`, `sessionId` and `severity` beginning with two asterisks, and `Read:` collapsed to a single `**` in the nine bundles that have one — **39 bullets present in the markdown and absent from the JSON.** Three defects in one regex: `lstrip(": ")` stripped the colon and not the closing `**`; a bullet-list field captured the empty remainder of its label line and stopped; and `Session:` went whole into `sessionId`, leaving `sessionBundle` null, which is the field split the schema exists to enforce undone by the thing populating it. Found by the parallel session reading the output, because **all four checkers reported 0 FAIL and all four were right** — the JSON was well formed, and things were merely missing. `0039` F16 records that every check in this repository compares two things that are present, and that *"completeness is not conformance"* has been a written rule since Revision 217 with nothing implementing it. D16 adds a `completeness` check to the verify group, deliberately not a second parser — a second reader shares the first's blind spots — asserting instead that no atomic value carries ornament, that a list in a document is a list of the same length in the data, and that every header field reaches a non-empty key. D17 makes the extractor single-use: it refuses to run once any generated-region marker exists in a file a generator writes, because parsing generated markdown back into the data that generated it can only lose and would look like a successful run. `0039`'s own findings.md is brought onto the current vocabulary — five passages naming `resolving`, `unresolved` and §§4b–4d as live, each now dated to what it was recorded against; the eight remaining uses are F10 arguing against the word and the rules table quoting `legend.md` as it stood, and neither is rewritten. **Fifth, sixth and seventh instruments in a row to fail against a healthy tree on their first run**: the completeness check at 18 false positives on prose fields whose schema amendment it had not read, and the one-way guard refusing because `state-as-data.md` documents its marker inside a fenced example.

**Revision 224** — supersedes Revision 223 and earlier. The four pre-D12 supersession successors get the clean slate D12 requires, and a byte comparison decided each one rather than a rule applied uniformly: `0037`, `0038` and `0040` carried `decisions.md` and `resolutions.md` **byte-identical to their predecessors' and headed with the predecessor's bundle name**, so those six files are removed and nothing is lost; `0036` loses a cloned `resolutions.md` and keeps a `decisions.md` its predecessor never had; `0041`'s pair is **moved to `0032` rather than deleted**, because `0032` had neither and both headers name it — a uniform operation would have destroyed the only copy of what `0032` decided. That also answers why every finding in those bundles read `framing` over accepted decisions: nothing had been decided in them. The decisions were taken in `0027`, `0028`, `0031` and `0032` against those bundles' framing, so `framing` was accurate and the borrowed `decisions.md` was what looked wrong. D15 requires a decision to cite at least one finding: the existing check asked whether every citation resolved and passed vacuously on a row with none, and 0039 D8 cannot void a decision that is under no finding. A first implementation exempted single-finding bundles on the strength of a convention Revision 219 had retired — a table is required even for one finding — and the owner's ruling removes the exemption. The check reports thirty rows in six bundles owned by two other sessions, and ten frozen rows in `0031` and `0032` are exempt by design; `--help` names the bundles and deliberately not the count, because a number written into a script goes stale and quoting a stale total is `0036`.

**Revision 223** — supersedes Revision 222 and earlier. `0039` D1 has been accepted since 2026-09-04 and never applicable: it named `.github/copilot-instructions.md` §§4b–4d as the home for procedure, and commit `1c48deb` deleted that file's sections before anyone acted on it. D13 replaces it — the principle was never in question, only the address. Procedure lives in `.github/session-management-instructions.md`, and D2's unfinished half is carried out four days late: the legend keeps three one-line definitions of the write categories and **the gating moves to §6**, which had been pointing back at the legend for it, so the two files agreed with each other and both disagreed with the decision. *Where a write is composed* leaves the legend as procedure duplicated verbatim in §6 — the section opened by arguing against its own presence — and *How the two meet* moves to §1 as structure. The legend loses 76 lines and holds vocabulary only. Two findings recorded and left open: F13, that the vocabulary file is named for a glance and read as a specification, with sixty-three citations making the rename a `0030`-class change that must follow the split rather than precede it; and F14, that nothing here is written for someone arriving cold, the de facto entry point being a 724-line reference manual — which is why nine rules have accumulated in a workspace file outside the repository, three of them today.

**Revision 222** — supersedes Revision 221 and earlier. The tag files are gone. A bundle's status and a session's state derive from `metadata.json` and are stored nowhere, which was the entire motivation for the JSON and which Revision 221 had left half-done — it added `declaredTag`, making three homes for a status where there had been two. §9 step 2 ran first: the derivation was computed for all 47 bundles and 7 sessions and compared against every tag in the tree, and **54 of 54 agreed** before a single file was deleted. Two corrections the comparison forced, neither an extraction bug: ownership is computed by scanning the session manifests as §6.1 always said, rather than read off a tag; and `unclaimed` means live work nobody holds, not merely "no owner", so a `resolved` bundle in no queue is not advertised as available. One defect it found in the tree: `0010` and `0024` were indexed `un-started` while listed by no session, and ladder row 1 sits above row 3. **The old checker could not have caught it** — it compared a tag against a row and both carried the same wrong value. `verify-findings-structure.sh` now derives from the data and compares the display against its source, which is the only comparison that can fail usefully. `state-as-data.md` §11.2 is answered: a rendered tag would have been a second copy of a derived fact, and the `ls` convenience did not survive being weighed against re-creating the defect the change removes.

**Revision 221** — supersedes Revision 220 and earlier. The finding lifecycle gains the transition rules it never had, and the session that wrote them had broken one two days earlier: `0039` F12 records that no arrow in the diagram names the event that causes it, so Revision 208 flattened twenty-one `reopened` rows while believing it had only read them, and every validator passed. D7 makes a status move on a material change to the record and never on a reading. D8 makes reframing invalidate the decisions under a finding, as a default that may be declined in writing. D9 gives Outcomes seven values with no word shared with any status vocabulary. D10 and D11 require a reason for reopening and for withdrawing. D12 makes supersession carry provenance as typed edges at finding granularity, gated on coverage and exclusivity. The first repair proposed for the twenty-one rows was `reopened` and was wrong for the same reason Revision 200 was: these six bundles are supersession successors, and `0038/F1` was never resolved — `0028/F1` was. `verify-doc-paths.sh` takes `0036` D1 option (iv) and reads `docs/` for the first time; the 38 references it surfaces are overwhelmingly records citing paths as they stood, so `historical` joins `proposed` as a declarable verdict, which is `0046`'s marker. Both broken anchors are repaired. `bin/verify-session-findings.sh` becomes the callsite for the framework's own checks, three of which move to `.internal/ai-scripts/session-management/` and one to `.share/`, which nothing had earned until now. 54 `metadata.json` files are extracted one way from the tree; the markdown stays authoritative, and the run found that §4.1's ban on ornament in a value needed an atomic/prose line drawn.
