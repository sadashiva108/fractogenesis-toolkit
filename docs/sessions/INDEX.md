...
|---|---|---|---:|---:|---|
| [`instruments-and-blind-spots-20260909-220203`](instruments-and-blind-spots-20260909-220203/) | `active` | **not yet filled** — the bundle precedes its session | [2](instruments-and-blind-spots-20260909-220203/findings-manifest.md) | 12 | **The instruments session.** `0050` and `0041` **assigned** at the owner's direction, both `unclaimed` since Revision 264. One bundle is instruments that are correct and cannot fire, the other a required shape nothing checks and **a check released unbuilt**. **`0041` carries five `decided` members with no work done on any** — two of the nineteen standing `decided` inside `unclaimed` bundles. **`0050` F3 is the emergency**: `extract-metadata.py` destroys 601 values across 38 fields and has no safe no-op. **The Owner column is empty on purpose.** Revision 286 |
