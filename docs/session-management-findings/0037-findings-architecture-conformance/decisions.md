...
| D9 | any `not recoverable` in a `metadata.md` lists the searches that came back empty | F7 | 2026-09-04 | `accepted` |

## Finding 1 — recording a findings bundle takes a revision

## D1 — §4b stands. §4c's contradicting sentence is deleted
...
## D2 — §4d's "always" binds a `prompt.md` for a session that is `unclaimed`, `owned` or `handoff`. A `closed` or `withdrawn` session's prompt is a record of what was given and is not retro-edited
...
wrong number gets noticed and a wrong unit gets believed. The check D4
...
## D6 — a findings bundle migrated into this shape from an already-closed record carries `resolutions.md` and no `decisions.md`, and its `resolutions.md` says so

Owner, 2026-09-03. One sentence in the rules; no file
...
D2 — and this is the same answer in a fourth place.
...
## D7 — four session states: `owned`, `handoff`, `closed`, `withdrawn`. `unclaimed` is dropped
...
## D9 — any `not recoverable` in a `metadata.md` lists the searches that came back empty
