...
| # | Finding | Status |
|---:|---|---|
| F1 | §4d instructs a session to mark bundles `superseded`; §4c never defines what that means or requires | `resolved` |
| F2 | Both existing accounts assume the superseding party owns the bundle. §4d itself creates the case where nobody does | `resolved` |
| F3 | Nothing states what a supersession must **not** touch, and all three prohibitions are load-bearing | `resolved` |
| F4 | The form that carries the pointer now exists in the tree and is written down nowhere | `resolved` |
