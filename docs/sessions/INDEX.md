...
|---|---|---|---:|---:|---|
| [`entity-model-and-vocabulary-20260909-053548`](entity-model-and-vocabulary-20260909-053548/) | `active` | **not yet filled** — the bundle precedes its session | [4](entity-model-and-vocabulary-20260909-053548/findings-manifest.md) | 36 | **The entity-model session, and the critical path.** `0037`, `0039`, `0045` and `0048` transferred in from `typed-bundles-architecture-20260908-204724` at Revision 261; all four stand `transferred` until its first write to each as owner. Carries `docs/architecture/typed-bundles-and-work.md` as a draft. **`0039` D23 and the relocation in `docs/ideas/the-collection-and-the-project.md` are both parked behind it**, on the owner's ordering — vocabulary and entities, then structure, then D23, then build. **The Owner column is empty on purpose** and the session fills it on its first write. **It derives `active` on day zero owning four bundles nobody has opened, which is `0045` F1's second live instance.** Revision 261 |
| [`drift-and-the-write-boundary-20260909-053548`](drift-and-the-write-boundary-20260909-053548/) | `active` | **not yet filled** — the bundle precedes its session | [3](drift-and-the-write-boundary-20260909-053548/findings-manifest.md) | 17 | **The instruments session.** `0038` and `0047` transferred in from `typed-bundles-architecture-20260908-204724` at Revision 261; **`0052` assigned** at the owner's direction, having stood `unclaimed` since Revision 252. Its subject is the tree's own state and the tree the owner commits from. **The boundary with the session above is the three rule documents**, which are that session's; sections 0 and 6 of the instruction set are writable here and announced in review. **The Owner column is empty on purpose.** Revision 261 |
| [`typed-bundles-architecture-20260908-204724`](typed-bundles-architecture-20260908-204724/) | `handoff` | Claude · `session_01QvZrvKpoKwCmyLNjzXzCdQ`, since 2026-09-09 | [0](typed-bundles-architecture-20260908-204724/findings-manifest.md) | 0 | **The architecture session.** Assigned `0037`, `0038`, `0045`, `0047`, `0048` on 2026-09-09 from the Revision 234 allocation. Carries `docs/architecture/typed-bundles-and-work.md` as a draft. **`0039` transferred in at Revision 248**, standing `transferred` until this session's first write to it as owner. **The Owner column was empty until Revision 239** — the bundle was created ahead of the session and the session filled its own row on its first write. **Handed off at Revision 261**: all six bundles to the two sessions above, `0052` assigned to the second, **nothing released to `unclaimed`**. `handoff-20260909-053548.md` is the record. Revisions 237, 239, 240, 247, 248, 252, 253, 256, 257, 259, 261 |
| [`assurance-coverage-20260908-204724`](assurance-coverage-20260908-204724/) | `closed` | Claude · `session_01Wd7US5C5Msr594vNidboLn`, to 2026-09-09 | [2](assurance-coverage-20260908-204724/findings-manifest.md) | 6 | **The assurance session.** Assigned `0040`, `0041`, `0042`, `0044`, `0046`, `0049` from the Revision 234 allocation; `0050`, `0051` and `0053` recorded here and assigned after. **Closed 2026-09-09 holding `0040` and `0044`, both `answered`; the other seven released to `unclaimed`** with their disposals in `final-summary.md`. Its answer is `0041` D1 — a check answers for the artifact as it will be read, not a curated proxy. Recorded `0054`, not owned, and the commission at `docs/ideas/the-collection-and-the-project.md`. Revisions 237, 251, 253, 254, 255, 258, 260, 261 |
