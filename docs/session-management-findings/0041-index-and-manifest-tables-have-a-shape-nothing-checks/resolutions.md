...
**Resolved:** 2026-09-09, 2026-09-10 for F6, and 2026-09-12 for F7
...
| F7 | D8 | **Adopted, not written here.** The recipe fix landed in two documents this bundle does not own: `.github/session-management-instructions.md` §0 step 3 under `0038` D10, and the same file's §6 recipe block under `0039` D28 — both now reading `git add -N .` then `git diff HEAD`. **Verified present at `4b0c95f` rather than taken from the decisions claiming it**: §0 step 3 at line 53, §6's block at line 595, and the superseded `git diff >` form surviving nowhere in the tree. What this bundle decided is that the remedy is adequate and complete; what it did not do is write it | 312 | — |
