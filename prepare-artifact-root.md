...
| `managed-inventory/`        | Company-managed component inventory before erase — MDM/Intune enrollment status, installed profiles, managed app bundles and package receipts, background managed services, and managed preference payloads. A run category: bundles under `runs/`, indexed in `MANIFEST.md`, resolved through `official/<context>.txt`. |
