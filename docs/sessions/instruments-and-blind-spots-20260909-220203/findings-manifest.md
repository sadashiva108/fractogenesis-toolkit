...
| 0050 | [`0050-instruments-that-cannot-fire-and-one-that-unmakes-the-record`](../../session-management-findings/0050-instruments-that-cannot-fire-and-one-that-unmakes-the-record/) | `session-management` | Three instruments are correct and cannot fire, and one unmakes the record | 7 | `analyzing` | **F3 and F6 `decided` at Revision 289.** Re-measured: **66 of 66 records, 740 values, 620 of them irrecoverable after `stamp`** — `genus` among them, so a run reverts Revision 271. D3 **retires the extraction half rather than guarding it**, because every way of making it safe is an enumeration and the enumeration is already one schema behind. The build is the next revision. F1 is the guard a bridged session is invisible to — **and `0045` F4 is the same guard read from the other side**, which is a `duplicates` or `co-decides` edge to settle, not a member to absorb |

**2 bundles · 13 members**, 10 of them live.
