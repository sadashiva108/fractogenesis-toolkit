...
Bring IntelliJ IDEA back to a working state for every project that mattered pre-image, without spraying secrets into unsafe paths and without overlaying stale machine-specific metadata onto the clean install. That means installing IntelliJ from the approved channel, importing the manual settings ZIP produced by Phase 2C rather than copying `options/` files by hand, restoring Scratches and Consoles and selected project `.idea` subtrees only from the matching IntelliJ version subtree, and treating HTTP Client environment files (which routinely embed bearer tokens and shared-secret headers) as encrypted-DMG material even when their `.json` extension makes them look ordinary.

This runbook owns:

```text
generating the IntelliJ-specific restore plan-note and sign-off checklist
installing IntelliJ IDEA and recording the install source
first-launch handling so the new install creates its own Application Support paths before restore
importing the manual settings ZIP produced by Phase 2C backup-apps (intellij subflow)
restoring Scratches, Consoles, and per-version config-copy content
restoring project-level .idea metadata selectively (runConfigurations, codeStyles, inspectionProfiles)
restoring non-secret HTTP Client request files (*.http, *.rest) from project-metadata
routing HTTP Client environment files (*.env.json, http-client.private.env.json) from the encrypted DMG only
validating project SDK, Gradle JVM, run configurations, and test builds
```
...
The first kind is IDE-level settings — keymaps, code style, inspection profiles, plugin list, editor behavior. The Phase 2C `intellij` subflow captures this into a `manual-settings-export/IntelliJ-settings-*.zip` file that IntelliJ can consume directly via `File → Manage IDE Settings → Import Settings`. This is the only IDE-level restore path used here; per-file overlays of `~/Library/Application Support/JetBrains/*/options/` are avoided because they routinely bring forward stale references after a major IntelliJ version bump.
...
| Variable | Meaning |
|---|---|
| `REIMAGE_ARTIFACT_ROOT` | Mounted external artifact volume that holds every pre-image backup and every post-image record. Required for `bin/restore-intellij.sh`. |
...
Skim the **Backup Sources**, **Detected IntelliJ Version Subtrees**, and **Secret-Bearing Sources** tables before continuing. If the version subtree table shows no `IntelliJIdea20YY.N/` rows, the Phase 2C IntelliJ subflow did not produce per-version content and Step 5 collapses to "settings ZIP only."
...
Be careful with full `config-copy/options/` overlays across a major version change. Prefer the settings ZIP (Step 4) first; overlay individual `options/` files only when something specific did not import.
...
| HTTP Client `*.env.json` | Sensitive. Restore only from encrypted DMG (Step 8). |
...
Check Git before committing anything in the project:
...
The `IntelliJIdea20YY.N/` directory under `~/Library/Application Support/JetBrains/` did not exist when Import Settings ran — IntelliJ silently ignored the write. Launch IntelliJ once (Step 3), quit, confirm the directory now exists, then reimport the ZIP.
...
If nothing is there, or the file is present but empty, redo Step 8 from the encrypted DMG.
