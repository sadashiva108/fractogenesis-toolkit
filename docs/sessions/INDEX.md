...
|---|---|---|---:|---:|---|
| [`instruments-and-blind-spots-20260909-220203`](instruments-and-blind-spots-20260909-220203/) | `active` | Claude · `session_01Fo6sBeux1JTyDzgKhCx2KU`, since 2026-09-10 | [2](instruments-and-blind-spots-20260909-220203/findings-manifest.md) | 21 | **The instruments session.** `0050` and `0041` **assigned** at the owner's direction, both `unclaimed` since Revision 264. One bundle is instruments that are correct and cannot fire, the other a required shape nothing checks and **a check released unbuilt**. **`0041` carries five `decided` members with no work done on any** — two of the nineteen standing `decided` inside `unclaimed` bundles. **`0050` F3 is the emergency**: `extract-metadata.py` destroys 601 values across 38 fields and has no safe no-op. **The Owner column was empty on purpose until this session's first write.** Revision 286 **First write Revision 288.** `0041` **D6 built and F6 `resolved`** — `counts` now sums the prose total beneath a table, and **its first run raised one this tree was carrying**: `docs/session-management-findings/INDEX.md` read 113 against 120 summed, hand-incremented 111 → 112 → 113 across Revisions 272, 278 and 282 while the column moved by seven more. D6 predicted a silent first run and was wrong, which is the better outcome. **`0050` F7 recorded**, from a relay by `entity-model-and-vocabulary-20260909-053548`: nothing compares a commit message's assertion against what the commit contains, measured on `636eba0`, `889b8ac` and `777f468`, and a mis-scoped subject **burns a revision number** — 285 is permanently unusable. The rule is §7's and that session is taking it; the instrument is `0050`'s and is not built ahead of the rule. **Revision 289**: `0050` **D3** decides F3 and F6 — the extraction half of `extract-metadata.py` is **retired rather than guarded**, on a re-measurement of **740 values destroyed and 620 irrecoverable**, `genus` among them. **Revision 292**: `0050` **F8 and F9** — this session reported two manifest gaps as undetected and permanent, **and was wrong on both counts**. `verify-manifest-coverage.sh` names 285 and 291 and prescribes the repair; **nothing runs it**. Across four commits `MISSING` went 0 → 2 and `ORPHANED` 3 → 4 while every check a session runs held its baseline. **Revision 294**: **F10** — the repair of a `MISSING` can only produce a permanent `ORPHANED`, so the metric argues against the act it prescribes; and **F7 extended** below the subject line, `15069fe` having carried an unfilled template placeholder into the log and attributed this session's Revision 289 to `drift-and-the-write-boundary-20260909-053548` in its trailer. **Revision 297 closes what this session owed**: `docs/ledgers/instrument-firing.md`, the written answer to which instruments can fire, as a ledger rather than a survey — **four run without being asked, six cannot produce an adverse verdict at all, and one is correct, fires and is reached by nothing**; `0050` F1 settled against `0045` F4 as two findings joined by `relates-to`; and a contribution to `0043` F13, `generalises` having opposite directions in two rank-4 copies, none in `docs/legend.md` and none in the code. **Revision 299 carries out D3**: `extract-metadata.py` deleted, `check-metadata-completeness.py` its own file, **`0050` F3 and F6 `resolved`**. The deletion made three unrunnable capabilities visible — the `unclaimed` derivation, the supersession gates, and a defective duplicate derivation — which is `0050` F8's class found by removing a thing rather than running it. **And `0041` F7**, found by the deletion itself: §6's patch recipe drops a deletion, so this revision's own patch carried 16 files of a 17-file change set until the file-list check caught it. **Revision 304**: `0041` **F8 and D7**, at Drift's request — §6.1 declares eleven projection classes and **nothing generates them**, `0043` D3 having ruled that a map rather than a plan. Measured: **5 disagreements in 612 rows, and all five sit in the three classes nothing compares**, against **0 in the 329 rows something checks**. D7 gives `structure` the two cheapest. `0050` F5's display, `framing` over a `resolved` member since Revision 255, is repaired. `docs/ledgers/projection-conformance.md` is the sweep — including its own four wrong first runs. **Revision 306 builds D7's precondition** — the derivation fixture set, per `0043` D3's *fixture set first, then a report-only checker*. **An enumeration over all 63 presence-combinations rather than chosen bundles**, which **answered `0043` F7**: the else branch does assert something, and this session's first attempt at stating it was wrong on seven. Suite 69 → 73. **Revision 307**: the last unattended projection row re-derived, and the ledger corrected — **`unclearable` and `unattended` are different conditions and its first table grouped them**, which is `0047` F9's shape inside the ledger built to prevent it. This session's *nobody is positioned to fix it* was wrong: **a projection belongs to its source, not its subject**, and `docs/sessions/INDEX.md` is shared by 12 of the last 15 revisions. **Surface now 1 of 613, and the one is unclearable.** **Revision 308 builds D7** — `structure` now compares a session manifest's `Standing` and a session's `State` against the data they derive from, 105 rows of §6.1 where it compared 56, `OK` 71 → 120. **The first run is silent and that is the weaker outcome**: Revision 307 cleared the last rows by hand, so Direction B is satisfied on constructed input — 4 raised, 4 real, 0 false, one of them a single `metadata.json` edit disagreeing with **two** displays at once. **And building it found `0050` F11**: the script carried a fourth copy of the standing derivation testing ownership before lineage — **128 disagreements in 384 enumerated cases**, unreachable in this tree, and it would have failed a conformant row for being right. **D4 deletes the copy**; the derivation is imported from the module that owns it and the suite gains the ordering assertion nothing made. Suite 73 → 74. **Revision 309 lands what this session composed at 299 and never applied.** `docs/ledgers/iris-conformance.md` existed in no tree while **two documents cited it as the place a correction had been written** — `projection-conformance.md` §34 and `IRIS-COORDINATION.md` §5 — and doc-paths reported one of them as a WARN nobody read. **A ledger whose subject is that measured figures go stale, held back nine revisions until its own did.** Landed as measured and corrected in a second entry rather than rewritten. The audit bundle lands as **`0057`**, twice-renumbered, `unclaimed`. **Seven of seven figures in the manual have now moved**; at 299 the bundle count still held. **`0038` F12 measured at `fc2836b` and contributed to Drift**: `--all` MISSING reads **8 in the checkout and 21 in a clone**, thirteen instances of one path — and the mechanism written down three times was wrong. `.internal/restore/` is **not untracked, it is empty**, which git cannot represent, **so no commit can ever close the gap**. **The default set reads 0 / 0 in both trees**, so F12 has never cost a session anything, and its remedy is `0012` D2, accepted at Revision 193 and never carried out. **And a third instance of `0050` F8**: `measure-iris-status.py` is correct, fires, and is reached by nothing — the first unrun instrument that *writes*, so its silence published wrong numbers under markers saying they were machine-derived. Regenerated here; still reached by nothing. **Revision 312 — the owner's test, and it reversed a call two sessions had agreed.** **A findings bundle must be able to reach a terminal standing; if it cannot, the reading is wrong.** Under the Revision 304 disposal `0041` could not: F7 was settled, correct and permanently `framing`, so resolving every other member still derived `analyzing`. **`0041` D8 reverses it**, adopting the remedy carried out under `0038` D10 and `0039` D28 — **verified present at `4b0c95f`** rather than taken from the decisions claiming it — and **F7 is `resolved`**. §11 turns out to be a dangling-citation rule rather than an ownership rule, and across **117 resolution rows none cites another bundle's decision**, so the adopt move is the only sanctioned shape. **`0041` F9 records what D8 does not repair**: nothing asserts reachability, it is not fully derivable, and F7's disposition sat in prose with `statusNote` null for eight revisions while every instrument read a bare `framing`. **`0050` F12** is the hand-off that vanished: `verify-findings-headers.sh` reads any `F<n>` in a resolutions row as local — three forms, all FAIL 12 against a baseline of 11 — handed here at Revision 301 and never recorded, **and D8's own row applies the workaround in the revision that records it**. |
| [`entity-model-and-vocabulary-20260909-053548`](entity-model-and-vocabulary-20260909-053548/) | `closed` | Claude · `session_01LSgzo7EtPPVJ1gG8s4NVNW`, 2026-09-09 to 2026-09-13 | [1](entity-model-and-vocabulary-20260909-053548/findings-manifest.md) | 9 | **The entity-model session, and the critical path.** `0037`, `0039`, `0045` and `0048` transferred in from `typed-bundles-architecture-20260908-204724` at Revision 261; **`0037` and `0039` cleared on this session's first writes** — F8 and F26 — and `0045` and `0048` still stand `transferred`. **`0043` transferred in at the owner's direction**, from `session-management-re-evaluation-20260906-110105`, which stands `handoff` with no successor: a schema decision closes deciding and only an owner may close it. It stays deliberately open. Carries `docs/architecture/typed-bundles-and-work.md` as a draft. **`0039` D23 and the relocation in `docs/ideas/the-collection-and-the-project.md` are both parked behind it**, on the owner's ordering — vocabulary and entities, then structure, then D23, then build. **The Owner column was empty until this session's first write.** **It derives `active` on day zero owning four bundles nobody has opened, which is `0045` F1's second live instance.** Revision 261 **Six findings recorded 2026-09-10** from measuring the code against the documents while writing `iris/`: `0043` F11-F13, `0037` F9, `0045` F4-F5. **`0043` and `0045` cleared their transfers** on those writes; `0048` alone still stands `transferred`. | **Closed 2026-09-13 at Revision 316, declared.** `0037` stands `answered` and is retained; **`0039`, `0043`, `0045` and `0048` were released to `unclaimed` with no successor named**, none of them having reached `answered`. The disposal, what stands open in each and what a taker should know are in [`final-summary.md`](entity-model-and-vocabulary-20260909-053548/final-summary.md).
| [`drift-and-the-write-boundary-20260909-053548`](drift-and-the-write-boundary-20260909-053548/) | `closed` | **not yet filled** — the bundle precedes its session | [3](drift-and-the-write-boundary-20260909-053548/findings-manifest.md) | 28 | **The instruments session.** `0038` and `0047` transferred in from `typed-bundles-architecture-20260908-204724` at Revision 261; **`0052` assigned** at the owner's direction, having stood `unclaimed` since Revision 252. Its subject is the tree's own state and the tree the owner commits from. **The boundary with the session above is the three rule documents**, which are that session's; sections 0 and 6 of the instruction set are writable here and announced in review. **The Owner column is empty on purpose.** Revision 261 |
