...
|---:|---|---|---|---:|---|---|
| 0037 | [`0037-findings-architecture-conformance`](../../session-management-findings/0037-findings-architecture-conformance/) | `session-management` | The findings-and-sessions architecture disagrees with itself and with the tree | 9 | `analyzing` | **The reading it was owed is recorded, as F8.** It arrived `answered` with all seven `resolved`; F8 is live, so the bundle is `analyzing` again — correct, and not a regression. F8 is `framing`: its wording is what `0039` D23 will carry, and D23 is parked |
| 0039 | [`0039-the-instruction-set-lags-the-rules-it-governs`](../../session-management-findings/0039-the-instruction-set-lags-the-rules-it-governs/) | `session-management` | The instruction set lags the rules it governs | 28 | `analyzing` | **The transfer cleared on F26**, which is D24's own rule applied to the write that recorded it. F24 and F25 arrived as contributions from `drift-and-the-write-boundary-20260909-053548` at Revision 268; **F26 is this session's and is `decided`** — the legend says a status moves on a write and also that reading is the transition, and the owner ruled for the write. The keystone and the most worked: F7, F12, F15, F16, F14 decided or resolved; fourteen still `framing`. **D23 is decided and deliberately not carried out** — the provenance extraction across the three rule documents, 23 retired read-trigger sites, parked until the entity model settles |
...
**5 bundles · 58 members**, 41 of them live.
