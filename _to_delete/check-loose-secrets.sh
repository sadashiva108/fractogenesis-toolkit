...
# Usage:
#   cd <repo-root>
#   chmod +x bin/check-loose-secrets.sh
#
#   # Check the configured artifact root
#   ./bin/check-loose-secrets.sh
#
#   # Check a specific root instead
#   ./bin/check-loose-secrets.sh --artifact-root /Volumes/Data/reimage-<asset>-<date>-open
#
...
#   ./bin/check-loose-secrets.sh --no-report
#
#   # List every path checked, not just the findings
#   ./bin/check-loose-secrets.sh --verbose
#
# Options:
  ...
  sort -t "$tab" -k6,6 -k5,5nr -k2,2 "$LEDGER_PATH" \
    | awk -F"$tab" -v run="$RUN_ID" -v when="$(date '+%Y-%m-%d %H:%M:%S')" '
      ...
      END {
        ...
        print ""
        print "Regenerated on every saved run of `bin/check-loose-secrets.sh`."
