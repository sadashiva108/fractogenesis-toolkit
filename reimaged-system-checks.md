...
```text
the individual restore evidence — Phase 6 through Phase 11 runbooks own their own captures
early post-image sanity checks and the initial "manual captures required" list — Phase 7 (record-first-boot)
pre-image validation — Phase 4B (reimage-prep-checks) runs the same bin/reimage-checklist.sh entrypoint with --phase pre
bulk personal-home restore — Phase 13 (restore-home), which runs after Phase 12 sign-off
```
...
Read this before running anything. Phase 12 is the *proof* phase, not another *capture* phase. The rebuilt Mac has already been through every restore that is going to happen before final sign-off (Phases 6–11); the goal here is to force each of those restores to declare itself with a machine-observable fact — a running daemon, an installed binary, a mounted volume, a Git config with two identities, a Docker CLI that talks to a daemon — and to name the ones that can only be validated by eye so they don't slip through.

The workflow depends on two facts working together: the checklist script covers the categories automation can cover, and the manual sign-off table covers the ones it cannot. Running the script once and stopping there is the most common Phase 12 mistake — the point is to run it, act on what it says, then run it again so the recorded artifact reflects the resolved state. The Phase 7 initial-post-image run wrote a `manual-captures-required.md` inside its own bundle; that file is the pre-flight for this phase, not this phase's output.

The post-image Time Machine backup lands at the *end* of Phase 12, not the beginning of Phase 13. The backup is what preserves the trusted rebuild before Phase 13 starts merging bulk personal content into it, so its ordering is deliberate.
...
Related script (Phase 7 first-boot bundle; its `manual-captures-required.md` is the pre-flight for this phase):
...
Pre-flight file written by Phase 7 (read, not written here):
...
| Flag | When to pass |
|---|---|
| `--phase post` | Required for Phase 12. Selects the post-image ruleset and the `reimaged-system/checklists/` output root. |
...
- Every Phase 6–11 runbook that is expected to run has finished, including any restore-notes plan-notes under `reimaged-system/restore-notes/`.
...
- The Phase 7 first-boot bundle exists under `reimaged-system/first-boot-*/`; its `manual-captures-required.md` is reachable.
...
- Are you running the *diagnostic* pass (expect WARN/FAIL rows and plan to act on them) or the *sign-off* pass (rows are expected to resolve or be documented, and the resulting file is the final artifact)? Every Phase 12 typically does both — the diagnostic run first, the sign-off run after fixes.
...
Before the automated checklist, resolve or acknowledge the manual rows Phase 7 already flagged:
...
For each outstanding row, either fix the underlying issue now, or record the deliberate skip in a fresh Phase 12 note under `restore-notes/` so the Phase 12 checklist has something to reference when it lands on the matching row.
...
|---|---|
| `PASS` | Move on. |
| `WARN` | Decide whether it blocks sign-off. If yes, act. If no, record why in a `restore-notes/` note so the next Phase 12 pass carries the reasoning. |
...
Record decisions in a Phase 12 note under `restore-notes/` — especially any manual `PASS` that would look ambiguous to a future reader.
...
Phase 12 is complete when the second checklist file exists, the manual sign-off table is fully resolved, and `tmutil latestbackup` points at a fresh post-image backup.
...
The Phase 12 run captures or checks:
